...
### 代理外部 API 接口
例如调用 OpenAI 的接口，你甚至不需要加 `/proxy/` 前缀，系统已全局自动识别：
```bash
# 方式1: 直接使用 /v1 (自动映射到 OpenAI)
curl -X POST "https://你的空间名.hf.space/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer sk-YOUR_KEY" \
  -d '{"model": "gpt-3.5-turbo", "messages": [{"role": "user", "content": "Hello!"}]}'

# 方式2: 携带完整目标URL (无缝透传，任意API均可)
curl -X POST "https://你的空间名.hf.space/https://api.anthropic.com/v1/messages" \
  -H "x-api-key: your-key" ...
```

### 代理 Google 搜索
直接在浏览器打开：
`https://你的空间名.hf.space/https://www.google.com`

由于我们**终极强化了跨域头和 OPTIONS 预检请求接管**，你可以直接在自己的 Vue/React 本地前端项目中，通过配置 `baseURL: 'https://你的空间名.hf.space/v1'` 来无缝联调大模型 API，再也不会出现 CORS 跨域错误！完美支持流式输出 (Stream)。
